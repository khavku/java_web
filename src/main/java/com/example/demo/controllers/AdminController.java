...
public class AdminController {
	...
	public String orderHandler(@ModelAttribute Orders order, Model model) {
		...
		// Cập nhật thông tin trong đối tượng Orders
		order.setTotalAmmout(totalAmount);
		order.setUser(user); // Bạn cần đảm bảo đối tượng `user` đã được định nghĩa và lấy từ session hoặc
								// database
		Date d = new Date();
		...
		// Thêm thông tin tổng số tiền và trạng thái thành công vào model
		model.addAttribute("amount", totalAmount);
		model.addAttribute("orderSuccess", true); // Thêm cờ để thông báo đã đặt hàng thành công
		List<Orders> orders = this.orderServices.getOrdersForUser(user);
		model.addAttribute("orders", orders);
		// Trả về lại trang "BuyProduct" với thông báo thành công
		return "BuyProduct";
	}
